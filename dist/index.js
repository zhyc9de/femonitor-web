                ...
                if (this.readyState === 4) {
                    if (this.status >= 200 && this.status < 300) {
                        ...
                        if (this._url !== self._options.report.url) {
                            self.safeEmitError(`${errorType}: ${this._url}`, TrackerEvents.reqError, reqErrorObj);
                        }
                    }
                }
            });
        ...
        Vue.config.errorHandler = (err, vm, info) => {
            ...
            try {
                if (vm) {
                    ...
                    this.safeEmitError(`${errorType}: ${errorMsg}`, TrackerEvents.vuejsError, errorObj);
                }
                else {
                    const errorObj = {
                        errorType: errorType,
                    ...
                    return originFetch(url, options)
                        ...
                        if (!this._isUrlInIgnoreList) {
                            if (status >= 200 && status < 300) {
                                ...
                                self.safeEmitError(`${errorType}: ${this._url}`, TrackerEvents.reqError, reqErrorRes);
                            }
                        }
...
class Monitor {
    ...
    listenMouseTrack() {
        myEmitter.on(TrackerEvents._mouseTrack, (event) => {
    ...
    listenBehaviors() {
        myEmitter.on(TrackerEvents._console, (behavior) => {
            this.pushBehavior(behavior);
            this.configData("_behavior", this.behaviorQueue, false);
        });
        myEmitter.on(TrackerEvents._clickEle, (behavior) => {
            this.pushBehavior(behavior);
            this.configData("_behavior", this.behaviorQueue, false);
        });
    }
    listenPerformanceInfo() {
        myEmitter.on(TrackerEvents.performanceInfoReady, (performanceInfo) => {
...
export { AjaxInterceptor, ConsoleType, device as DeviceUtil, Env, ErrorObserver, ErrorType, FetchInterceptor, Monitor, NavigationType, PerformanceObserver, Reporter, TrackerEvents, defaultTrackerOptions };
